...
description:
  "Audit and plan system package manager migrations (e.g. Homebrew to MacPorts),
  including inventory, categorization, migration planning, and cleanup."
version: 1.0.0
author: Operator 11O
license: MIT
platforms: [macos]
metadata:
  hermes:
    tags: [macOS, Migration, Homebrew, MacPorts, mise, Package-Manager, Audit]
...
2. **MacPorts names might not exist yet.** A port can lag behind the latest
   upstream release. Always verify with `port search` before declaring a
   mapping.

3. **PATH ordering matters.** MacPorts lives in `/opt/local/bin`; system tools
   in `/usr/bin`. Ensure MacPorts is before system but after mise shims:
   `mise > macports > homebrew > system`.
...
5. **mise vs. MacPorts for language runtimes.** Prefer mise: it supports
   `.tool-versions` files, per-project overrides, and is cross-platform. Only
   use MacPorts for system libraries (openssl, readline, sqlite).

6. **Do not mix Homebrew and MacPorts for the same library.** Linker conflicts
