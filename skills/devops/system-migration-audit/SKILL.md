...
description: "Audit and plan system package manager migrations (e.g. Homebrew to MacPorts), including inventory, categorization, migration planning, and cleanup."
version: 1.0.0
author: Operator 11O
license: MIT
platforms: [macos]
...
| `mise-has`    | Better managed by mise (asdf-style)  | nodejs, python, ruby, rust   |
...
1. **MacPorts builds from source by default.** Large packages (LLVM, Chromium)
   can take 30+ minutes. Use `port install -b` to attempt binary archives
   first.
...
5. **mise vs. MacPorts for language runtimes.** Prefer mise: it supports
   `.tool-versions` files, per-project overrides, and is cross-platform.
   Only use MacPorts for system libraries (openssl, readline, sqlite).

6. **Do not mix Homebrew and MacPorts for the same library.** Linker
   conflicts will silently break builds. Pick one, stick to it, and unlink
   the other.
