...
- MUST NOT “fix” security by disabling protections (e.g., turning off CSP to
  ...
  CORS, skipping sanitization, or “temporary” bypasses that ship). ([OWASP Cheat
  Sheet Series][2])
...
- Any data from cross-window messaging: `window.postMessage` payloads. ([OWASP
...
### 3.2 Browser-enforced protections
...
- SHOULD enable clickjacking defenses via `frame-ancestors` (CSP) and/or
  `X-Frame-Options`, unless embedding is an explicit product requirement. ([MDN
  Web Docs][8])

### 3.3 High-risk features baseline (MUST if used)

- If rendering any user-provided HTML/markdown/rich text:
  - MUST sanitize before insertion and avoid raw DOM sinks. ([OWASP Cheat Sheet
    Series][9])
...
### REACT-CONFIG-001: Never embed secrets in client bundles
...
### REACT-XSS-001: Avoid `dangerouslySetInnerHTML` on untrusted content
...
- OWASP explicitly calls out React’s `dangerouslySetInnerHTML` without
  sanitization as a common framework “escape hatch” pitfall. ([OWASP Cheat Sheet
  Series][9])
...
### REACT-XSS-002: Rely on React escaping by default
...
### REACT-DOM-001: Avoid DOM XSS sinks in React code
...
### REACT-URL-001: Validate untrusted URLs used for navigation
...
### REACT-TT-001: Use Trusted Types with CSP where feasible
...
- MDN describes Trusted Types as a way to ensure input is transformed (commonly
  ...
  (`innerHTML`, `document.write`) and JS URL sinks (`script.src`). ([MDN Web
  Docs][3])
...
### REACT-CSP-001: Deploy and maintain a CSP
...
- OWASP describes CSP as “defense in depth” against XSS and notes it can help
  enforce SRI even on static sites, but should not be the only defense. ([OWASP
  Cheat Sheet Series][2])

---

### REACT-SRI-001: Use Subresource Integrity or self-host
...
### REACT-3P-001: Minimize and govern third-party JavaScript
...
### REACT-AUTH-001: Protect tokens and sessions from XSS
...
### REACT-CSRF-001: CSRF-protect cookie-authenticated requests
...
- OWASP CSRF guidance explains SameSite behavior (Lax/Strict/None) as a
  ...
  balance, but it is not a complete substitute for CSRF protections. ([OWASP
...
### REACT-NET-001: Prevent data exfiltration via outbound requests
...
### REACT-SW-001: Service workers require HTTPS and safe caching rules
...
- “Secure contexts” exist to prevent MITM attackers from accessing powerful
  APIs; service workers are an example of such a powerful feature. ([MDN Web
  Docs][18])

---

### REACT-HEADERS-001: Ensure essential security headers for the React shell
...
### REACT-POSTMSG-001: Validate `postMessage` origin and payloads
...
### REACT-FILE-001: Uploaded files and previews must avoid active content
...
### REACT-SUPPLY-001: Frontend dependency and supply-chain hygiene
...
- OWASP NPM security guidance recommends enforcing the lockfile and explicitly
  ...
  option to use `--ignore-scripts` to reduce attack surface. ([OWASP Cheat Sheet
  Series][22])
...
- Trusted Types API —
  `https://developer.mozilla.org/en-US/docs/Web/API/Trusted_Types_API` ([MDN Web
  Docs][3])
...
- npm audit docs — `https://docs.npmjs.com/cli/v10/commands/npm-audit/` ([npm
  Docs][20])
- npm ci docs — `https://docs.npmjs.com/cli/v10/commands/npm-ci/` ([npm
  Docs][21])
