...
- Store only opaque identifiers (e.g., session ID) or non-sensitive state in the
  cookie; store sensitive session state server-side. ([OWASP Cheat Sheet
  Series][8])
...
- SHOULD use a proven CSRF approach (synchronizer token pattern, or
  well-reviewed middleware) rather than rolling your own. ([OWASP Cheat Sheet
  Series][2])
...
- SHOULD deploy a CSP when serving HTML that includes any user content. ([OWASP
...
- If CORS is needed:
  ...
  - MUST NOT combine credentialed requests with wildcard origins (this is unsafe
    and commonly rejected by compliant middleware). ([OWASP Cheat Sheet
    Series][6])
...
- If subprocess is necessary:
  ...
  - SHOULD use strict allowlists for any variable component ([OWASP Cheat Sheet
    Series][22])
...
- SHOULD set timeouts and carefully control redirects. ([OWASP Cheat Sheet
  Series][23])
...
- Session/cookies + CSRF:
  - `SessionMiddleware(` and cookie flags (`https_only`, `same_site`)
    ([PyPI][5])
  - POST/PUT/PATCH/DELETE handlers using cookie auth with no CSRF checks ([OWASP
...
- Injection:
  - SQL strings with f-strings/concatenation into `.execute(...)` ([OWASP Cheat
    Sheet Series][21])
...
- OWASP API Security Top 10 (2023) —
  `https://owasp.org/API-Security/editions/2023/en/0x11-t10/` ([OWASP
  Foundation][13])
