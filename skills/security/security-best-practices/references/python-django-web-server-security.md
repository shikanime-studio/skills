...
running `manage.py check --deploy` against production settings. ([Django
Project][1])
...
- SHOULD separate dev vs prod settings modules, with safe defaults for
  production (fail closed if critical settings are missing). ([Django
  Project][1])
...
- SHOULD load it from env or a file/secret store (not hard-coded). ([Django
  Project][1])
...
- SHOULD consider enabling:
  - `SECURE_SSL_REDIRECT = True` (with correct proxy config) ([Django
    Project][3])
...
- Add HSTS carefully (start with low value, validate, then increase). Django
  warns misconfig can break your site for the HSTS duration. ([Django
  Project][3])
...
- MUST only set `SECURE_PROXY_SSL_HEADER` if you control the proxy (or have
  ...
  misconfig can compromise security and lists required conditions. ([Django
  Project][3])
...
- MUST set `SESSION_COOKIE_SECURE=True` (only transmit over HTTPS). ([Django
  Project][3])
- MUST keep `SESSION_COOKIE_HTTPONLY=True` (Django default is `True`). ([Django
  Project][3])
...
- SHOULD set `CSRF_COOKIE_SECURE=True` when using HTTPS/TLS. ([Django
  Project][3])
...
- MUST include `{% csrf_token %}` in internal POST forms; MUST NOT include it in
  forms that POST to external URLs (Django warns this leaks the token). ([Django
  Project][4])
...
- When deployed with HTTPS, Django’s CSRF middleware also checks the Referer
  header for same-origin (Django security docs mention this). ([Django
  Project][2])
...
- MUST use Django ORM/querysets for normal DB access; Django notes querysets are
  parameterized and protected from SQL injection under typical use. ([Django
  Project][2])
- MUST be very careful with raw SQL; if using `raw()`, `cursor.execute()`,
  ...
  warn to escape user-controlled parameters using `params`. ([Django
  Project][7])
...
### DJANGO-UPLOAD-001: Uploaded files must be validated and served safely
...
- MUST enforce size limits (at least at the web server; Django security docs
  recommend limiting upload size at the server to prevent DoS). ([Django
  Project][2])
...
- SHOULD consider serving uploads from a separate top-level/second-level domain
  ...
  and note that a subdomain may be insufficient for some protections. ([Django
  Project][2])
...
### DJANGO-PATH-001: Prevent path traversal in static and media files
...
### DJANGO-HEADERS-001: Enable essential security headers
...
- SHOULD use `django.middleware.security.SecurityMiddleware` and configure it
  ...
  - COOP (Django setting `SECURE_CROSS_ORIGIN_OPENER_POLICY`, default
    `'same-origin'`). ([Django Project][3])
  - HTTPS redirects and HSTS as appropriate (see DJANGO-HTTPS-001). ([Django
    Project][3])
...
- Add/enable middleware and configure the settings intentionally. ([Django
  Project][3])
...
- SHOULD deploy a CSP to mitigate XSS and content injection classes; Django’s
  security docs recommend CSP and note it is new in Django 6.0. ([Django
  Project][2])
- MUST understand CSP limitations:
  - Avoid excluding routes from CSP coverage; Django warns that an unprotected
    page can undermine protected pages due to same-origin policy. ([Django
    Project][2])
...
- Implement a realistic CSP, ideally report-only first, then enforce. ([Django
  Project][3])

---

### DJANGO-AUTH-001: Password storage must use secure hashers
...
- Enable password validators appropriate to the product’s risk profile. ([Django
  Project][3])
...
- MUST configure production logging deliberately; Django’s deployment checklist
  explicitly calls out reviewing logging before production. ([Django
  Project][1])
...
### DJANGO-SUPPLY-001: Dependency and patch hygiene
...
- Identify Django version; compare against latest supported release (Django’s
  download page publishes current stable and supported branches). ([Django
  Project][9])
...
- Deployment/dev server:
  - `manage.py runserver`, `runserver 0.0.0.0`, `--insecure` ([Django
    Project][1])
...
- XSS:
  - `|safe`, `autoescape off`, `mark_safe(`, HTML string concatenation ([Django
    Project][5])
