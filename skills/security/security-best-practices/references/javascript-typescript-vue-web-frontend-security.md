...
- SHOULD deploy `X-Content-Type-Options: nosniff`, plus other headers as
  appropriate (Referrer-Policy, Permissions-Policy). ([OWASP Cheat Sheet
  Series][4])
...
- If loading scripts/styles from a CDN, SHOULD use Subresource Integrity
  (`integrity` attribute) with appropriate `crossorigin` configuration. ([MDN
  Web Docs][11])
...
- Vite Docs: Env Variables and Modes (VITE\_\* exposure + security notes) —
