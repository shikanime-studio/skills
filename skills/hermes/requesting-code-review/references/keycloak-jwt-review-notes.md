...
Use these checks when reviewing NestJS auth changes that add Keycloak JWT
support.
...
- Keep JWKS retrieval off the hot path where possible.
  - If the code fetches JWKS on cache miss, require a timeout and a clear
    failure mode.
  - Prefer a dedicated client/fetch service over inline network calls inside
    auth policy code.
