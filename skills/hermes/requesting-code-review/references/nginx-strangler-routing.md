...
This repo uses `nginx-strangler` as the single API entrypoint in local migration
work.
...
- if requests appear to "all hit NestJS", verify the client proxy target and the
  local server ports before changing routing rules
- if strangler cannot forward traffic, verify `host.docker.internal` resolution
  from inside the container; Rancher Desktop may differ from Docker Desktop here
...
4. If needed, inspect the resolved nginx config and upstreams inside the
   container.
