...
- Configuration derivation belongs in the configuration spec, not the consumer
  spec.
  - Example: `getKeycloakIssuer()` and `getKeycloakCertsUrl()` belong with
...
- Prefer `describe('<ClassName>')` and `it('should ...')` wording that states
...
- A test suite mixes config derivation, network transport, and domain logic in
  the same file.
